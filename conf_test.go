...
func TestConf_FillByTag_customTypeWithBuiltinKind(t *testing.T) {
	chk := assert.New(t)
	//
	s := `
	global-value = I'm a global!
...
`
	...
	type T struct {
		Global           SpecialString `conf:"global-value"`
		GlobalWithSpaces SpecialString `conf:"global with spaces"`

		// Tests [section]
		Section struct {
			SectionValue SpecialString `conf:"section-value"`
		} `conf:"section"`
		SectionPtr *struct {
			SectionValue SpecialString `conf:"section-value"`
		} `conf:"section"`
		...
		// Tests [schedule]
		Schedule struct {
			Archive []SpecialString `conf:"archive"`
		} `conf:"schedule"`
	}
	testIt := func(t *T) {
		// pointers created
		...
		chk.NotNil(t.PtrSectionSlicePtr)
		// global config
		chk.Equal(SpecialString("I'm also global!"), t.Global)
		chk.Equal(SpecialString("yay!"), t.GlobalWithSpaces)
		// section config
		chk.Equal(SpecialString("I'm a section value!"), t.Section.SectionValue)
		chk.Equal(SpecialString("I'm a section value!"), t.SectionPtr.SectionValue)
		// slice and ptr-to-slice sections
		chk.Equal(1, len(t.SectionSlicePtr))
		chk.Equal(SpecialString("I'm a section value!"), t.SectionSlicePtr[0].SectionValue)
		chk.Equal(1, len(*t.PtrSectionSlicePtr))
		chk.Equal(SpecialString("I'm a section value!"), (*t.PtrSectionSlicePtr)[0].SectionValue)
		//
		chk.Equal(2, len(t.Schedule.Archive))
		chk.Equal(SpecialString("0 6 * * *"), t.Schedule.Archive[0])
		chk.Equal(SpecialString("0 */2 * * *"), t.Schedule.Archive[1])
	}
